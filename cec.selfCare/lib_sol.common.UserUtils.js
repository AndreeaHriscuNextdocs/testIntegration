...
sol.define("sol.common.UserUtils", {
   ...
   * @param {Array} groupNames Names of the groups
   */
  removeUsersFromGroups: function (userNames, groupNames) {
    var me = this,
        groupIds = [],
        newUserInfos, groupName, group, userInfos, userInfo, membershipIds, groupId, i, j, index;
    ...
    for (i = 0; i < groupNames.length; i++) {
      ...
      try {
        group = ixConnect.ix().getUserNames([groupName], CheckoutUsersC.BY_IDS_RAW)[0];
        groupIds.push(parseInt(group.id, 10));
      } catch (ex) {
        // ignore
      }
    ...
    userInfos = ixConnect.ix().checkoutUsers(userNames, CheckoutUsersC.BY_IDS_RAW, LockC.NO);
