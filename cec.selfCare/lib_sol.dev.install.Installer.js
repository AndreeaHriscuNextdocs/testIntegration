...
sol.define("sol.dev.install.functions.CheckWfIsRunning", {
  ...
  check: function () {
    ...
    return (me.config.install.checkWfIsRunning != false);
  },
...
sol.define("sol.dev.install.functions.AdjustDocMasks", {
  ...
  updateDocMask: function (docMaskDiffData) {
    ...
    for (i = 0; i < docMaskDiffData.lines.length; i++) {
      ...
      me.handler.log({ text: "Create document mask line: docMask={0}, lineKey={1}, data={2}", data: [docMaskDiffData.maskName, docMaskLineData.key, docMaskLineDataString] });
...
sol.define("sol.dev.install.mixin.RefUtils", {
  ...
  /**
   * Marks unreferenced source Sords
   * @param {String} objId
   */
...
sol.define("sol.dev.install.functions.InstallIxPlugins", {
  ...
  initialize: function (config) {
    ...
    me.$super("sol.dev.install.InstallFunctionBase", "initialize", [config]);
    me.ixPluginsDirPath = me.handler.payloadDirPath + File.separator + "IX Plug-ins";
    me.ixPluginsDir = new File(me.ixPluginsDirPath);
    me.ixPluginFiles = me.ixPluginsDir.listFiles();
  },
  ...
  execute: function () {
    ...
    for (i = 0; i < me.ixPluginFiles.length; i++) {
      ...
      try {
        ...
        me.handler.log({ text: "IX plug-in '" + ixPluginFileName + "' installed." });
      } catch (ex) {
        exceptionString = sol.common.ExceptionUtils.parseException(ex);
        me.handler.log({ text: "Can't install IX plug-in: name=" + ixPluginFileName + ", exception=" + exceptionString });
        continue;
      }
...
sol.define("sol.dev.install.functions.ExecuteRegisteredFunctions", {
  ...
  execute: function () {
    ...
    if (me.config.install.executeRegisteredFunctions.reloadScripts) {
      ...
      me.handler.log({ text: "connProps=" + me.conn.connProperties.toString() });
...
sol.define("sol.dev.install.functions.ShiftDates", {
  ...
  getManifestCreatedDate: function (manifestFilePath) {
    ...
    date = Packages.org.apache.commons.lang.time.DateUtils.parseDate(value, ["yyyy-MM-dd'T'HH:mm:ssXXX"]);
    isoDate = Packages.org.apache.commons.lang.time.DateFormatUtils.format(date, "yyyyMMddHHmmss") + "";
